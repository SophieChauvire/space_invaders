...
class jeu:
    ...
    def __init__(self, window, length, height, speed = 1,cadence_tir = 4000):
        ...
        self.walls = Walls(self.canvas,self)
    ...
    def start(self): # Est appelée pour commencer à jouer, supprime les éléments du menu et lancer les methodes qui permettent le bon déroulement du jeu
        ...
        self.canvas.after(16, self.player.player_movement)
        ...
        self.canvas.after(16, self.player.no_tir)
        self.window.bind('<q>', self.player.move_left)
        self.window.bind('<Q>', self.player.move_left)
        self.window.bind('<d>', self.player.move_right)
        self.window.bind('<D>', self.player.move_right)
        self.window.bind('<KeyRelease>', self.player.stop_move)
        self.window.bind('<space>', self.player.nouveau_tir)
    ...
    def niveau_screen(self): # Ecran de transition entre deux manches
        ...
        self.niveau_label = tk.Label(self.canvas, textvariable=self.niveau_SV, fg='#FFE213', bg='black', font='Helvetica 60 bold')
    ...
    def end_jeu(self): # Ecran de jeu over
        ...
        label = tk.Label(self.canvas, text='jeu OVER', fg='white', bg='black')
